...
class TestNequIP:
    ...
    def testCreatePureMLSystem(self, platform_int):
        ...
        potential = MLPotential("nequip", modelPath=os.path.join(test_data_dir, "toluene", "toluene.ckpt"), lengthScale=self._LENGTH_SCALE, energyScale=self._ENERGY_SCALE)
    ...
    def testCreateMixedSystem(self, platform_int):
        ...
        potential = MLPotential("nequip", modelPath=os.path.join(test_data_dir, "toluene", "toluene.ckpt"), lengthScale=self._LENGTH_SCALE, energyScale=self._ENERGY_SCALE)
        mixedSystem = potential.createMixedSystem(prmtop.topology, mmSystem, mlAtoms, interpolate=False, mlLongRange=False)
        interpSystem = potential.createMixedSystem(prmtop.topology, mmSystem, mlAtoms, interpolate=True, mlLongRange=False)
        platform = mm.Platform.getPlatform(platform_int)
